              ...
              <vs-table :data="list">
                ...
                <template slot-scope="{data}">
                  <vs-tr :key="indextr" v-for="(tr, indextr) in data">
                    ...
                    <vs-td v-if="tr.cate != null" :data="tr.id">
                    ...
                    <vs-td :data="tr.id">
                      <router-link :to="{name:'edit_type_cate',params:{id:tr.id}}">
                        <vs-button
                          vs-type="gradient"
                          size="lagre"
                          color="success"
...
export default {
  ...
  methods: {
    ...
    listTypeCategory() {
      this.loadings(true);
      this.listTypeCate({ keyword: this.keyword })
      .then(response => {
          this.loadings(false);
    ...
    destroy(){
      this.loadings(true);
      this.destroyTypeCate(this.id_item)
      .then(response => {
        this.listTypeCategory()
        this.loadings(false);
