              ...
              <vs-table max-items="5" pagination :data="list">
                ...
                <template slot-scope="{data}">
                  <vs-tr :key="indextr" v-for="(tr, indextr) in data">
                    ...
                    <vs-td v-if="tr.home_title != null" :data="tr.home_title">{{tr.home_title}}</vs-td>
                    <vs-td v-if="tr.home_title == null" :data="tr.home_title">-----</vs-td>
                    <vs-td :data="tr.id">
                      <vs-avatar size="70px" :src="tr.avatar" />
                    </vs-td>
                    <vs-td :data="tr.status">
                      {{ tr.status == 1 ? 'Trên thanh Menu' : 'Không trên thanh Menu'}} - {{ tr.quiz_id == 1 ? 'Danh mục sản phẩm trang chủ' : 'Không hiện danh mục sản phẩm trang chủ'}}
                    </vs-td>
                    <vs-td :data="tr.id">
